...
struct Batch: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        Supported step commands:
          tap, swipe, gesture, touch, text, button, key, key-sequence, key-combo

        Batch-only pseudo-step:
          sleep <seconds>

        Examples:
          taptaptap batch --udid SIMULATOR_UDID --step "tap --id BackButton" --step "text 'hello'"
          taptaptap batch --udid SIMULATOR_UDID --file steps.txt
          cat steps.txt | taptaptap batch --udid SIMULATOR_UDID --stdin
        """
    )
