...
struct Slider: AsyncParsableCommand {
    ...
    /// Resolved once at the top of `run()`; read by the private helpers below (which are not
    /// passed the UDID explicitly to keep this diff minimal against the pre-Phase-4 shape).
    ...
    mutating func run() async throws {
        ...
        simulatorUDID = try await resolveSimulatorUDID(udid, logger: logger)
    ...
    private func parseNormalizedAXValue(_ rawValue: String?) throws -> Double {
        guard let rawValue else {
            throw CLIError(errorDescription: "Matched slider does not expose a numeric AXValue, so TapTapTap cannot deterministically set it.")
        }
