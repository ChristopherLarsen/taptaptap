    ...
    func setup(logger: AxeLogger) async throws {
        ...
        do {
            ...
            if developerDirectory.isEmpty {
                ...
                throw CLIError(
                    errorDescription: "TapTapTap could not find an active Xcode installation. Select Xcode with `xcode-select` or set `DEVELOPER_DIR`, then try again."
                )
            }
        ...
        do {
            ...
            throw CLIError(
                errorDescription: "TapTapTap could not load simulator support from the selected Xcode installation. Confirm Xcode 26 or later is selected and try again."
            )
        }
    }
}
