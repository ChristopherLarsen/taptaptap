...
struct AccessibilityFetcher {
    /// `nested` selects the serialized JSON shape: `false` (idb's default) returns a flat array
    /// (each element serialized independently, no `children`); `true` returns the nested tree.
    /// Internal element-resolution callers (`fetchAccessibilityElements`, used by --label/--id
    /// targeting and batch/orientation code that walks `children`) always pass `true` regardless
    /// of this parameter's default — only the direct `describe-all`/`describe-point` JSON output
    /// commands vary it from their own `--nested` flag.
    static func fetchAccessibilityInfoJSONData(
        ...
        return try await retryingAfterTestManagerRecovery(
            ...
            if let point {
                // idb never wires `--filter` onto describe-point (confirmed against
                // `AccessibilityInfoAtPointCommand` in `idb/cli/commands/accessibility.py`), so
                // this path ignores `filter` entirely, same as before this phase.
                return try await fetchAccessibilityInfoJSONData(from: target, at: point, nested: nested)
            }
            return try await fetchFrontmostAccessibilityInfoJSONData(from: target, nested: nested, filter: filter)
        }
    }
    ...
    private static func fetchAccessibilityInfoJSONData(
        ...
        try await retryingTransientPointFallback(at: point) {
            ...
            return try serializedAccessibilityData(from: accessibilityElement, nested: nested)
        }
    }
    ...
    private static func fetchFrontmostAccessibilityInfoJSONData(
        ...
        for attempt in 0..<5 {
            ...
            let data: Data
            do {
                data = try serializedAccessibilityData(from: accessibilityElement, nested: nested)
            ...
            latestData = data
            // The warm-up check always runs on the UNFILTERED data: a sparse screen that
            // filters down to one kept element must not look like an unwarmed tree and burn
            // every retry. The filter is applied only to the winning result, afterward.
            if try containsAccessibilityDescendant(in: data) {
                return try filter.applied(toJSON: data)
            }
            if attempt < 4 {
        ...
        return try filter.applied(toJSON: latestData)
    }
    ...
    static func recoverTestManagerDaemon(
        ...
        guard status == 0 else {
            throw CLIError(
                errorDescription: "TapTapTap could not restore accessibility automation for simulator \(simulatorUDID). Restart the simulator and try again."
            )
        }
    ...
    static func runProcess(
        ...
        if process.isRunning {
            terminateProcess(process)
            throw CLIError(errorDescription: "TapTapTap timed out while restoring accessibility automation.")
        }
    ...
    private static func serializedAccessibilityData(from accessibilityElement: FBAccessibilityElement, nested: Bool) throws -> Data {
        let response = try accessibilityElement.serialize(
            with: FBAccessibilityRequestOptions(nestedFormat: nested, keys: accessibilityRequestKeys)
        )
        return try serializeAccessibilityInfo(addingCompatibilityDefaults(to: response.elements))
    }

    /// Whether the frontmost-application response already reflects more than just the bare root
    /// element (used to detect a not-yet-warmed-up accessibility tree and retry). In nested format
    /// this is "the (single) root has non-empty `children`"; in flat format (no `children` key
    /// anywhere) it is "the array has more than the one root element" — both mean the same thing:
    /// the traversal reached descendants.
    static func containsAccessibilityDescendant(in data: Data) throws -> Bool {
        ...
        guard let root = array.first, let children = root["children"] as? [Any] else {
            return false
        }
        return !children.isEmpty
    }
    ...
    static func serializeAccessibilityInfo(_ accessibilityInfo: Any) throws -> Data {
        guard accessibilityInfo is [String: Any] || accessibilityInfo is [[String: Any]] else {
            throw CLIError(errorDescription: "TapTapTap received an unsupported accessibility response from the simulator.")
        }
    ...
    // This key set preserves TapTapTap's legacy public JSON schema; update it only with explicit schema coverage.
    ...
    // TapTapTap's former IDB serializer did not return traits. Xcode 27's private nested serializer also
