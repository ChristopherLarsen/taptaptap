...
enum ElementResolutionError: LocalizedError, UserFacingError {
    ...
    var errorDescription: String? {
        ...
        case .multipleMatches(let count, let kind, let value, let hasUniqueIDs):
            if hasUniqueIDs {
                if kind == "--id" {
                    return "Multiple (\(count)) accessibility elements matched \(kind) '\(value)'. Add --element-type to narrow the match, or use coordinates. \(tip)"
                }
    ...
    var userFacingDescription: String {
        errorDescription ?? "TapTapTap could not resolve the requested accessibility element."
    }
}
...
struct AccessibilityTargetResolver {
    static let describeUITip = "Make sure the app is on the expected screen, then run `taptaptap ui describe-all` and prefer --id when available."
