...
extension Tap: BatchConvertible {
    ...
    func toBatchPrimitives(context: BatchContext, logger: AxeLogger) async throws -> [BatchPrimitive] {
        ...
        case .simulator:
            let tapEvent = duration.map { FBSimulatorHIDEvent.tapAt(x: physicalPoint.x, y: physicalPoint.y, duration: $0) }
                ?? FBSimulatorHIDEvent.tapAt(x: physicalPoint.x, y: physicalPoint.y)
...
extension Key: BatchConvertible {
    func toBatchPrimitives(context: BatchContext, logger: AxeLogger) async throws -> [BatchPrimitive] {
        ...
        if control { modifiers.append(224) }
        if option { modifiers.append(226) }
        if shift { modifiers.append(225) }
        if command { modifiers.append(227) }
        if tab { modifiers.append(43) }
        ...
        if !modifiers.isEmpty {
            ...
            return [.hidMergeable(.composite(events))]
        }
        ...
        return [.hidMergeable(.shortKeyPress(UInt32(key)))]
    }
}
...
extension Text: BatchConvertible {
    func toBatchPrimitives(context: BatchContext, logger: AxeLogger) async throws -> [BatchPrimitive] {
