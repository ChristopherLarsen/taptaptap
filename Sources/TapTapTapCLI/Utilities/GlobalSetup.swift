...
func performGlobalSetup(logger: AxeLogger) async throws {
    ...
    do {
        ...
        if xcodePath.isEmpty {
            let errorMessage = "TapTapTap could not find an active Xcode installation. Select Xcode with `xcode-select` or set `DEVELOPER_DIR`, then try again."
        ...
        let errorMessage = "TapTapTap could not resolve the active Xcode installation: \(error.localizedDescription)"
        logger.error().log(errorMessage)
        throw CLIError(errorDescription: errorMessage)
    }
    ...
    do {
        ...
        let errorMessage = "TapTapTap could not load simulator support from the selected Xcode installation: \(error.localizedDescription)"
        logger.error().log(errorMessage)
        throw CLIError(errorDescription: errorMessage)
    }
