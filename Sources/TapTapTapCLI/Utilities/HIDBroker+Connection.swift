...
struct HIDBrokerNotReadyError: LocalizedError, UserFacingError {
    let userFacingDescription = "TapTapTap could not establish simulator input. Wait for the simulator to finish booting and try again."
...
extension HIDBroker {
    private static let diagnosticLogger = Logger(
        subsystem: "com.taptaptap.hidbroker",
        category: "HIDBroker"
    )
    ...
    /// Sweeps the broker root directory (`$TMPDIR/taptaptap-hid-<uid>/`) for endpoint files left
    /// behind by a broker that exited without its own cleanup running -- a crash, SIGKILL, or a
    /// developer-directory/UDID identity nobody reconnects to again (F3, TESTING.md: 13 stale
    /// socket/lock files found after a 200-cycle soak with 0 running brokers). `.lock` and
    /// `.lifetime.lock` files are never unlinked on ordinary exit (they are reusable flock
    /// mutexes by design -- their presence alone is not staleness), and a `.sock` only gets
    /// removed lazily by the one client that later tries to reconnect to that exact identity. Age
    /// alone is not a safe discriminator (a slow-but-live broker's socket could be older than any
    /// reasonable bound); liveness is checked directly: a lock is stale only if it can be
    /// non-blocking-flocked (no live holder), a socket only if connecting to it fails with
    /// ECONNREFUSED/ENOENT (nothing listening). Called once per broker process startup, not on
    /// every command, so the directory scan and any probe connects are rare.
    ///
    /// `excludingIdentity`, when given, skips every entry named `<identity>[...]` -- the caller's
    /// own about-to-be-(re)created endpoint. This is a narrowing, not a correctness requirement
    /// (a genuinely live holder is never pruned regardless, since liveness is still the only
    /// discriminator): it avoids a self-inflicted, self-recovering race where sweeping a file this
    /// same broker is a heartbeat away from creating could momentarily present a detached inode to
    /// a concurrent client's own liveness check, causing an extra (harmless but wasteful) spawn.
    static func pruneStaleEndpoints(rootDirectory: String, excludingIdentity currentIdentity: String? = nil) {
        guard let entries = try? FileManager.default.contentsOfDirectory(atPath: rootDirectory) else { return }
        for entry in entries {
            if let currentIdentity, entry.hasPrefix(currentIdentity) { continue }
            let path = (rootDirectory as NSString).appendingPathComponent(entry)
            if entry.hasSuffix(".lock") {
                pruneStaleLockFile(at: path)
            } else if entry.hasSuffix(".sock") {
                pruneStaleSocketFile(at: path)
            }
        }
    }

    /// Matches `.sock.lock` (the startup lock) and `.sock.lifetime.lock` (the liveness lock) --
    /// both end in `.lock`. Removing a file while holding its own exclusive flock is safe: the
    /// unlink only detaches the name, a concurrent racer either already holds the same lock (our
    /// flock attempt fails first, so we never reach here) or opens a fresh file afterward.
    private static func pruneStaleLockFile(at path: String) {
        var info = stat()
        guard lstat(path, &info) == 0,
              (info.st_mode & S_IFMT) == S_IFREG,
              info.st_uid == getuid(),
              info.st_mode & (S_IRWXG | S_IRWXO) == 0 else { return }
        let descriptor = Darwin.open(path, O_RDWR | O_CLOEXEC | O_NOFOLLOW)
        guard descriptor >= 0 else { return }
        defer { Darwin.close(descriptor) }
        guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else { return } // a live process holds it
        _ = unlink(path)
        _ = flock(descriptor, LOCK_UN)
    }

    /// A closed probe connection resolves fast even against a live broker on a different identity
    /// (its accept/handshake/read either errors or hits immediate EOF once we close), the same
    /// probe idiom `waitForStaleEndpointShutdown` already uses for the caller's own identity.
    private static func pruneStaleSocketFile(at path: String) {
        guard (try? socketIdentity(at: path)) != nil else { return }
        do {
            let descriptor = try connect(to: path)
            Darwin.close(descriptor) // reachable: a live broker owns it, leave it alone
        } catch {
            let nsError = error as NSError
            guard nsError.domain == NSPOSIXErrorDomain,
                  nsError.code == Int(ECONNREFUSED) || nsError.code == Int(ENOENT) else { return }
            try? removeOwnedSocket(path)
        }
    }
