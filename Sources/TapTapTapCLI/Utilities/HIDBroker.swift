...
enum HIDBroker {
    static let inputDeliveryFailureDescription = "TapTapTap could not deliver simulator input. The simulator may have restarted or disconnected. Confirm it is booted and try again."
    ...
    static func serve(simulatorUDID: String, logger: AxeLogger) async throws {
        ...
        while true {
            ...
            // Authenticate the peer before anything is written to it, including the readiness handshake
            // (taptaptap 3d NOTE N7). The socket is already 0600 inside a 0700 directory.
    ...
    private static func handle(
        ...
        do {
            ...
            for primitive in request.primitives {
                ...
                case .down, .up:
                    let (x, y) = try touchCoordinates(primitive)
    ...
    /// True when the connected peer runs as this process's user.
    static func isSameUserPeer(_ descriptor: Int32) -> Bool {
        var peerUID: uid_t = 0
        var peerGID: gid_t = 0
        return getpeereid(descriptor, &peerUID, &peerGID) == 0 && peerUID == getuid()
    }

    /// The coordinates of a touch primitive; missing or non-finite values are rejected.
    static func touchCoordinates(_ primitive: HIDBrokerPrimitive) throws -> (x: Double, y: Double) {
        ...
        guard x.isFinite, y.isFinite else {
            throw CLIError(errorDescription: "Touch primitive coordinates must be finite.")
        }
        return (x, y)
    }
    ...
    static func endpointPath(simulatorUDID: String, developerDirectory: String) throws -> String {
        ...
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            ...
            .appendingPathComponent("taptaptap-hid-\(uid)", isDirectory: true)
        ...
        let developerDirectory = URL(fileURLWithPath: developerDirectory, isDirectory: true)
            ...
            .resolvingSymlinksInPath()
            .path
        // Combined into one 64-bit hash (not two) to keep headroom under sockaddr_un.sun_path: the
        // NUL separator makes ("a", "bc") and ("ab", "c") hash differently. Uniqueness only needs to
        // hold per-user (control of the environment is not a security boundary), so one full-strength
        // hash beats two truncated ones. 3f: found the two-hash form left only 2 bytes of margin
        // (102/104) against a real TMPDIR before the "taptaptap-hid-" rename even widened the prefix.
        let identity = String(fnv1a64(simulatorUDID + "\0" + developerDirectory), radix: 16)
        ...
        let filename = "\(identity)-v\(protocolVersion).sock"
    ...
    private static func spawnBroker(simulatorUDID: String) throws {
        guard let executable = Bundle.main.executableURL else {
            throw CLIError(errorDescription: "Unable to locate the TapTapTap executable for the HID broker.")
        }
        let process = Process()
