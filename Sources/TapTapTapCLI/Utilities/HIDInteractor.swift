...
struct HIDInteractor {
    ...
    /// Can be set via TAPTAPTAP_HID_STABILIZATION_MS environment variable; capped at 1000 ms.
    private static var stabilizationDelayMs: UInt64 {
        if let envValue = ProcessInfo.processInfo.environment["TAPTAPTAP_HID_STABILIZATION_MS"],
    ...
    static func makeSession(for simulatorUDID: String, logger: AxeLogger) async throws -> Session {
        ...
        do {
            ...
            throw CLIError(
                errorDescription: "TapTapTap could not initialize simulator input using the selected Xcode installation. Confirm Xcode 26 or later is selected and try again."
            )
        }
        ...
        guard HIDBroker.shouldReuseSession(
            ...
            throw CLIError(
                errorDescription: "Simulator \(simulatorUDID) restarted while TapTapTap was connecting. Try the command again."
            )
        }
        try await HIDBroker.waitForHIDReadiness(
