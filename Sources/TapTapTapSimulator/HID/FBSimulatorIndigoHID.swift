...
public final class FBSimulatorIndigoHID {
  ...
  public func touchScreenSize(
    ...
    return try touchMessage(point: point, direction: direction)
  }
  ...
  public func twoFingerTouchScreenSize(
    ...
    // The patches below write up to 0x184 + 8: refuse a SimulatorKit allocation too small for them
    // instead of writing past it (taptaptap 3d NOTE N3, same guard as the single-touch path).
    let requiredSize = 0x184 + MemoryLayout<Double>.size
    let messageSize = malloc_size(bytes)
    guard messageSize >= requiredSize else {
      free(message)
      throw FBSimulatorHIDError.indigoMessageTooSmall(size: messageSize, required: requiredSize)
    }
  ...
  private func touchMessage(point: CGPoint, direction: FBSimulatorHIDDirection) throws -> Data {
    var point = point
    ...
    // The patches below write up to 0x44 + 8 and read the IndigoTouch at 0x30: refuse a SimulatorKit
    // allocation too small for them instead of writing or reading past it (taptaptap 3d NOTE N3).
    let requiredSize = max(0x44 + MemoryLayout<Double>.size, 0x30 + MemoryLayout<IndigoTouch>.size)
    let sourceSize = malloc_size(sourceBytes)
    guard sourceSize >= requiredSize else {
      free(source)
      throw FBSimulatorHIDError.indigoMessageTooSmall(size: sourceSize, required: requiredSize)
    }
