...
struct AccessibilityFetcherTests {
    ...
    func cancellationStopsRecoveryProcess() async throws {
        ...
        // A broken cancellation waits out the 10 s `sleep`; 5 s keeps that discriminating
        // without failing on a loaded host (taptaptap 3e: 1 s flaked at 1.08 s under parallel builds).
