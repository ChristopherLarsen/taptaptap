...
struct AccessibilityTargetResolverTests {
    ...
    @Test("Ambiguous --id match points to ui describe-all and does not suggest --id again")
    func ambiguousIDMatchDoesNotSuggestIDAgain() throws {
        let roots = try decodeElements(
            """
            [
              {
        ...
        #expect {
            _ = try AccessibilityTargetResolver.resolveTapPoint(roots: roots, query: .id("duplicate-id"))
        } throws: { error in
            let message = (error as? ElementResolutionError)?.userFacingDescription ?? ""
            return message.contains("Multiple (2) accessibility elements matched --id 'duplicate-id'")
                && message.contains("Add --element-type to narrow the match, or use coordinates.")
                && !message.contains("Use --id when labels are not unique")
                && message.contains("taptaptap ui describe-all")
                && !message.contains("describe-ui --udid")
        }
    }

    @Test("Ambiguous --label match still suggests --id, via ui describe-all")
    func ambiguousLabelMatchSuggestsID() throws {
        let roots = try decodeElements(
            """
            [
              {
        ...
        #expect {
            _ = try AccessibilityTargetResolver.resolveTapPoint(roots: roots, query: .label("Duplicate Label"))
        } throws: { error in
            let message = (error as? ElementResolutionError)?.userFacingDescription ?? ""
            return message.contains("Multiple (2) accessibility elements matched --label 'Duplicate Label'")
                && message.contains("Use --id when labels are not unique.")
                && message.contains("taptaptap ui describe-all")
        }
    }
