...
struct CLIErrorTests {
    ...
    func axeRuntimeErrorsAreUserFacing() {
        ...
        #expect(
            String(describing: VideoProcessingError.failedToDecodeImage)
                == "TapTapTap could not decode a simulator video frame."
        )
        #expect(
        ...
        #expect(
            String(describing: HIDBrokerNotReadyError())
                == "TapTapTap could not establish simulator input. Wait for the simulator to finish booting and try again."
        )
        #expect(
    ...
    func missingSimulatorErrorIsActionable() {
        ...
        #expect(error.description == "No simulator with UDID EXAMPLE-UDID was found. Run `taptaptap list-targets` to see available simulators.")
