...
struct TestHelpers {
    ...
    /// Get the path to the taptaptap binary using #file to find source root
    static func getAxePath(testFile: String = #file) throws -> String {
        if let axeBinPath = ProcessInfo.processInfo.environment["TAPTAPTAP_BIN_PATH"], !axeBinPath.isEmpty {
            if FileManager.default.fileExists(atPath: axeBinPath) {
                return axeBinPath
            }

            throw TestError.unexpectedState("TAPTAPTAP_BIN_PATH points to a missing taptaptap binary at \(axeBinPath). Please run 'swift build'.")
        }
        ...
        if let srcRoot = ProcessInfo.processInfo.environment["SRC_ROOT"] {
            ...
            // <root>/Tests/TapTapTapTests/<file>.swift -> <root>
            sourceRoot = URL(fileURLWithPath: testFile)
        ...
        // taptaptap 3e: SwiftPM's default debug bin path, without spawning `swift build --show-bin-path`.
        let axePath = URL(fileURLWithPath: sourceRoot)
            .appendingPathComponent(".build/debug/taptaptap")
            .path
        if FileManager.default.fileExists(atPath: axePath) {
            return axePath
        }

        throw TestError.unexpectedState("taptaptap binary not found at \(axePath). Please run 'swift build'.")
    }
